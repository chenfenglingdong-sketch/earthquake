/*
 * Copyright (C) 2023 Lee. All Rights Reserved.
 */
define(['./Circle',
        './Cylinder',
        './LayerManager',
...
// 设置初始位置为上海
wwd.navigator.lookAtLocation.latitude = 31.2304; // 上海的纬度
wwd.navigator.lookAtLocation.longitude = 121.4737; // 上海的经度
wwd.navigator.range = 7000; // 观察点到地球表面的距离，单位为米

// 调用 redraw() 来应用这些更改
wwd.redraw();
// 设置高分辨率地形模型
var elevationModel = new WorldWind.EarthElevationModel();
wwd.globe.elevationModel = elevationModel;
...
// 其余初始化代码保持不变...
	...
	// 添加 Bing 地图图层
var bingLayers = new WorldWind.BingAerialLayer("AqaxDqbWK50LQwySSIR5nfbu-0QkWA5NCbavP-KRm3AzEvSFcjK9Q7ISpLqUUaU7", "zh-cn");
wwd.addLayer(bingLayers);
...
// 检查当前使用的 Globe
console.log("Current Globe:", wwd.globe);

// 检查当前使用的 ElevationModel
console.log("Current ElevationModel:", wwd.globe.elevationModel);
